...
const createTokensPair = require('../utils/createTokensPair');
...
const handleRefreshToken = async (req, res) => {
  try {
    const decoded = jwt.verify(req.body.refreshToken, req.secret);
    ...
    const {
      accessToken,
    } = createTokensPair(payload, req.secret);
    return res.send({
      cookie: `${config.accessTokenName}=${accessToken}; path=/`,
      user,
    });
  } catch (err) {
    return res.status(401).send(err);
  ...
  switch (req.passwordCheck) {
    ...
    default:
      ...
      const {
        accessToken,
        refreshToken,
      } = createTokensPair(user, req.secret);
      return res.send({
        cookie: `${config.accessTokenName}=${accessToken}; path=/`,
        refreshToken: `${config.refreshTokenName}=${refreshToken}; path=/`,
        user,
      });
  }
};
